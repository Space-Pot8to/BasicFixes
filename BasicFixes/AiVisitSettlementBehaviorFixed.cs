...
namespace BasicFixes
{
    ...
    {
		public AiVisitSettlementBehaviorFix(bool isEnabled) : base(isEnabled)
        {
			if (isEnabled)
			{
		...
		public class AiVisitSettlementBehaviorReplacement : CampaignBehaviorBase
		{
                    ...
                    foreach (KeyValuePair<ValueTuple<float, int>, Settlement> keyValuePair2 in sortedList)
                    {
                        ...
                        else
                        {
                            ...
                            if (item < 1f && (mobileParty.UnlimitedWage || mobileParty.TotalWage < mobileParty.PaymentLimit))
                            {
                                ...
                                {
                                    num32 = (float)ApproximateNumberOfVolunteersCanBeRecruitedFromSettlement(leaderHero, value2);
			...
			private void CalculateVisitHideoutScoresForBanditParty(MobileParty mobileParty, Settlement currentSettlement, PartyThinkParams p)
			{
                ...
                int num3 = 0;
                IReadOnlyList<Hideout> allHideouts = Settlement.All.Where(x => x.IsHideout).Select(x => x.Hideout).ToList();
